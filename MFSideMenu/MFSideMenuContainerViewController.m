...
- (void)setMenuState:(MFSideMenuState)menuState completion:(void (^)(void))completion {
    ...
    switch (menuState) {
        case MFSideMenuStateClosed: {
            ...
            UINavigationController *navigation=(UINavigationController*)self.centerViewController;
            if([[navigation topViewController] isKindOfClass:[WSViewController class]])
            {
                WSViewController *controller=(WSViewController*)[navigation topViewController];
                if(controller.isScroll==NO)
                {
...
- (void) setLeftSideMenuFrameToClosedPosition {
    ...
    leftFrame.size.height=[[UIScreen mainScreen]bounds].size.height-20;
